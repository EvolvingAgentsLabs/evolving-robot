...
def _stub_dream(work_dir, new_body, reason):
    def dream_fn():
        skill = get_skill(load_skills(work_dir), PATROL)
...
def main() -> int:
    ...
    try:
        ...
        baseline_body = get_skill(load_skills(tmp), PATROL).body
        ...
        good_body = baseline_body + "\n## Efficiency\n\nObserve once per checkpoint, not per step.\n"
        r2 = ctrl.evolve_step(
            dream_fn=_stub_dream(tmp, good_body, "observe once per checkpoint (demo-good)"),
            score_fn=_score_seq(1.0, 1.0),
            target_skill=PATROL,
        )
        print(f"   evolved={r2.evolved} kept={r2.kept} rolled_back={r2.rolled_back}")
        kept_ok = get_skill(load_skills(tmp), PATROL).body == good_body.strip()
        print(f"   evolved skill kept on disk: {kept_ok}")

        frozen_oid, artifact = ctrl.freeze(message="freeze: verified patrol skill set")
...
#   ctrl.evolve_step(dream_fn=lambda: dream.dream("patrol-route"),
#                    score_fn=lambda: run_patrol_success_rate())   # a live odyssey mission
